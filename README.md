...
```bash
npm install
npm run dev          # http://localhost:5173, diagnostic endpoints served from fixtures
npm run dev:live     # same, but calls the real API at VITE_API_BASE_URL
```

The backend is a separate FastAPI service. Until it exists, `npm run dev` answers
the diagnostic endpoints from `src/mocks` — `.env.development` sets `VITE_USE_MOCKS=1`.

| Variable             | Purpose                                                         |
| -------------------- | --------------------------------------------------------------- |
| `VITE_API_BASE_URL`  | Base URL of the API. Empty means same-origin.                     |
| `VITE_USE_MOCKS`     | `1` starts the MSW worker in the browser. Set in `.env.development`. |

`public/mockServiceWorker.js` is generated by `npx msw init public` and must stay
committed — without it the worker cannot register and API calls fall through to
the dev server as 404s.
